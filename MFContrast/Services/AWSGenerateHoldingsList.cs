...
namespace MFContrast.Services
{
    ...
    public interface IAWSGenerateHoldingsList
    {
        public bool DownloadComplete { get; set; }
        Task<List<Holding>> CreateAsync(string keyName);
        bool DownLoadCsvFiles(IEnumerable<string> fileNames);
    }
...
{
BucketName = Constants.AWS.BUCKET_NAME.ToLowerInvariant(),
Key = string.Format("{0}/{1}", Constants.AWS.FOLDER_NAME.ToLowerInvariant(), filename)
};
try
{
using (var response = await S3Client.GetObjectAsync(request))
{
string avatarFilepath = System.IO.Path.Combine(App.PersonalFolderPath, filename);
